...
async function setupSemanticSearch(
  ...
  const existingKey = getEmbeddingKey();
  if (existingKey) {
    console.log('');
    console.log(
      styleText('green', 'Semantic search') +
        (existingKey === 'local'
          ? ' — local embeddings configured. Ready.'
          : ' — embedding key found. Ready.'),
    );
    return;
  }
  ...
  console.log(
    '  relevant documentation by meaning, not just keywords. This can use local',
  );
  console.log(
    '  embeddings or an embedding API key. Without it, agents can still',
  );
  console.log(
    '  use ' +
  ...
  if (!rl) {
    console.log(
      styleText('yellow', '  No embedding provider configured.') +
        ' Set LAT_EMBEDDING_PROVIDER=local, set LAT_LLM_KEY env var, or run ' +
        styleText('cyan', 'lat init') +
        ' interactively.',
    );
    return;
  ...
  console.log(
    '  Choose local embeddings to keep search on this machine. You can switch',
  );
  console.log(
    '  to a hosted key later with ' + styleText('cyan', 'LAT_LLM_KEY') + '.',
  );
  console.log(
    '  Local default: ' +
      styleText('cyan', DEFAULT_LOCAL_EMBEDDING_MODEL) +
      ' (' +
      DEFAULT_LOCAL_EMBEDDING_DIMENSIONS +
      'd)',
  );
  ...
  console.log('  1. Local embeddings (recommended)');
  console.log('  2. Hosted embedding API key');
  console.log('  3. Skip for now');
  const choice = await prompt(rl, `  Choose semantic search provider [1]: `);

  if (!choice || choice === '1' || choice.toLowerCase() === 'local') {
    saveLocalEmbeddingConfig();
    console.log(
      styleText('green', '  Local embeddings enabled') +
        ' in ' +
  ...
  if (choice === '3' || choice.toLowerCase() === 'skip') {
    console.log(
      styleText('dim', '  Skipped.') +
        ' You can set ' +
        styleText('cyan', 'LAT_EMBEDDING_PROVIDER=local') +
        ' or ' +
  ...
  if (choice !== '2' && choice.toLowerCase() !== 'hosted') {
    console.log(
      styleText('yellow', '  Unrecognized choice.') +
